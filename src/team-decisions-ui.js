...
function priorityPercent(item){return Math.round(clamp(35+item.impact*115,0,99));}
function marketTarget(item,name){
 const reqs=item.requirements||[],primary=reqs[0]||{key:'pa',label:'Atributo',minimum:0};
 window.MZMarketTarget={formationName:name,role:item.role,position:item.position,slotIndex:item.index,slotCode:item.code,mainKey:primary.key,mainLabel:primary.label,minimumMain:primary.minimum,requirements:reqs,minimumRating:item.targetRating,playerUid:item.player.uid,current:item.currentRating,teamBottleneck:item.bottleneck.key,expectedTeamGain:item.gain};
}
function renderNeedsTeam(){
 ...
 view.innerHTML=`<div class="needs-toolbar"><div><span class="eyebrow">PLAN DE CONTRATACIONES</span><h2>¿Qué jugador debo comprar?</h2><p>Ahora la prioridad nace del problema del equipo completo, no del jugador con menor nota.</p></div><label class="needs-select">Analizar para<select id="needs-formation">${options}</select></label></div>
 <div class="decision-summary"><div class="decision-box"><span>CUELLO DE BOTELLA</span><strong>${esc(d.bottleneck.label)} · ${d.bottleneck.value.toFixed(1)}/10</strong><p>${esc(d.bottleneck.detail)}</p></div><div class="decision-box"><span>MEJOR INTERVENCIÓN</span><strong>${esc(n.code)} · ${esc(n.position)}</strong><p>Mejorar este perfil produce la mayor ganancia colectiva estimada dentro de ${esc(d.name)}.</p></div></div>
 <article class="team-buy-card"><div class="team-buy-head"><div><span class="eyebrow">COMPRA RECOMENDADA · ${esc(d.name)}</span><h2>${esc(n.position)}</h2><p>Referencia actual: ${esc(n.player.name)} · ${n.currentRating.toFixed(1)}/10. No se recomienda por ser simplemente el más débil, sino por cuánto puede cambiar el funcionamiento del XI.</p></div><div class="team-buy-score"><span>PRIORIDAD</span><strong>${priorityPercent(n)}%</strong></div></div>
 <div class="decision-deltas">${deltaHtml(n.deltas)||'<b>Mejora principalmente la profundidad del equipo</b>'}</div>
 ...
 <button id="team-buy-market" class="ghost-btn team-buy-action">Buscar candidatos del mercado</button></article>`;
...
function patchDashboard(){
 ...
 const t=document.getElementById('metric-need'),s=document.getElementById('metric-need-detail');if(t)t.textContent=d.bottleneck.label;if(s)s.textContent=`${d.first.code} · ${d.first.position} · mejora potencial ${d.first.gain>=0?'+':''}${d.first.gain.toFixed(2)}`;
}
function renderFunctionalBench(lineup){
 const wrap=document.getElementById('bench-list');if(!wrap)return;const bench=D.functionalBench(players,lineup);
 ...
 const help=wrap.closest('.bench-panel')?.querySelector('.help');if(help)help.textContent='Banco funcional: portero, cobertura, organizador, amplitud y rematador.';
}
...
function decorateMarket(){
 ...
 const parts=Object.entries(sim.componentDeltas||{}).filter(([,v])=>Math.abs(v)>=.02).sort((a,b)=>Math.abs(b[1])-Math.abs(a[1])).slice(0,4);box.innerHTML=parts.map(([k,v])=>`<span class="${v>0?'up':''}">${esc(D.COMPONENT_SHORT[k]||k)} ${v>=0?'+':''}${v.toFixed(2)}</span>`).join('')+(sim.rawGain>0&&sim.gain===0?`<span class="note">Mejora total +${sim.rawGain.toFixed(2)}: demasiado pequeña para justificar compra</span>`:'');foot.appendChild(box);card.dataset.teamDecisions='1';});
}
function patchYouth(){
 ...
 const panel=document.createElement('article');panel.className='panel youth-functions';panel.innerHTML=`<div class="panel-head"><div><span class="eyebrow">PROYECCIÓN FUNCIONAL</span><h3>Qué función puede cubrir cada juvenil</h3></div></div><div class="youth-function-grid">${list.map(x=>`<div class="youth-function-card"><span>${esc(x.label)}</span><strong>${esc(x.p.name)}</strong><small>${x.score.toFixed(1)}/10 · puede reducir una futura compra si progresa</small></div>`).join('')}</div>`;view.appendChild(panel);
}
function installUI(){
 ...
 registerView('needs',['Qué comprar','Detecta qué fichaje mejora más al equipo completo.'],renderNeedsTeam);
 ...
 document.addEventListener('mz:main-lineup-changed',()=>{needFormation='';setTimeout(()=>{patchDashboard();patchYouth();decorateMarket();},0);});
 document.addEventListener('mz:roster-changed',()=>{needFormation='';simulationCache.clear();setTimeout(()=>{patchDashboard();patchYouth();},0);});
 setTimeout(()=>{patchDashboard();patchYouth();decorateMarket();},0);
}
installUI();
window.MZTeamDecisionsUI={renderNeeds:renderNeedsTeam,patchDashboard,renderFunctionalBench,decorateMarket,patchYouth,decision};
})();
