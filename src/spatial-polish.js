...
// 1) Recupera el desempate por banco y conserva la marca de formación completa.
...
// 2) Al recargar la tabla normal, conserva Pie, altura y peso ya enriquecidos.
...
// 3) Parser más tolerante para perfiles copiados desde ManagerZone.
...
// 4) Muestra Pie, altura y peso dentro de la ficha del jugador.
if (typeof openPlayer === 'function') {
  ...
  openPlayer = function openPlayerWithProfile(uid){
    ...
    if(head && !head.dataset.profileExtra){head.dataset.profileExtra='1';head.innerHTML += ` · <b>${parts.map(esc).join(' · ')}</b>`;}
  };
}
...
const avg=(arr,key)=>arr.length?arr.reduce((s,p)=>s+(Number(p?.[key])||0),0)/arr.length:0;
function tacticalRecommendation(){
  const ctx=lineupContext();if(!ctx)return null;
  const {slots,lineup}=ctx;
  const pass=(avg(lineup,'pa')+avg(lineup,'ctrl'))/2;
  ...
  const tackling=avg(lineup,'en'),stamina=avg(lineup,'res'),fitness=avg(lineup,'ef');
...
// 5) Resultado financiero con signo y estado visual distinto para pérdida/ganancia.
