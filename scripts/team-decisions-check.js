...
const bench=D.functionalBench(roster,lineup);
assert.equal(bench.length,5,'el banco funcional debe cubrir cinco funciones');
assert.equal(new Set(bench.map(x=>x.player.uid)).size,bench.length,'el banco funcional no debe repetir jugadores');
for(const label of ['Portero','Defensa / cobertura','Organizador','Banda / amplitud','Rematador'])assert(bench.some(x=>x.label===label),`falta cobertura ${label}`);
...
assert.equal(tiny.gain,0,'una mejora táctica mínima no debe justificar una compra');
...
assert(ui.includes('CUELLO DE BOTELLA')&&ui.includes('Banco funcional')&&ui.includes('saleCandidates'),'la UI debe integrar necesidad, banco y ventas');
console.log('✓ Team-wide needs, market threshold, functional bench and sale protection OK');
